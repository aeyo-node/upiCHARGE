...
<body>
  ...
  const handleProceedToPayment = () => {
    if (!customerMobile || customerMobile.trim().length !== 10 || isNaN(customerMobile)) {
      setErrorErrorMsg("Please enter a valid 10-digit mobile number.");
      return;
    }
    setErrorErrorMsg("");
    setScreen("payment");
    ...
    try {
      const res = await fetch(`${apiBase}/api/charging/stop`, {
        ...
        body: JSON.stringify({
          charger_id: chargerId,
          customer_mobile: customerMobile,
          prepaid_amount: activeSession.prepaid_amount
        })
      });
  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-start p-4 max-w-md mx-auto relative pb-24">
      {/* Header Bar */}
      <header className="w-full flex items-center justify-between py-6 border-b border-white/5 mb-6">
        <div className="flex items-center space-x-2">
          ...
          <span className="font-semibold text-lg tracking-tight">upiCHARGE<span className="text-apple-accent">.com</span></span>
        </div>
        <div className="flex items-center space-x-1 glass py-1 px-3 rounded-full text-[11px] text-apple-emerald font-medium tracking-wide">
      ...
      {/* ERROR BANNER */}
      {errorMsg && (
        <div className="w-full glass border-l-4 border-l-apple-rose p-4 rounded-2xl mb-6 flex items-start space-x-3 text-sm animate-pulse-slow">
          ...
          <div className="flex-1">
            ...
            <p className="text-white/80 mt-1">{errorMsg}</p>
          </div>
        </div>
      )}
      ...
      {screen === "home" && (
        <div className="w-full flex flex-col items-center space-y-6 animate-fadeIn">
          <div className="text-center space-y-2 mt-4">
            <h1 className="text-4xl font-extrabold tracking-tight bg-gradient-to-b from-white to-white/70 bg-clip-text text-transparent">
              Tap & Charge
            </h1>
            <p className="text-sm text-apple-silver font-light px-6">
          ...
          <div className="w-full glass rounded-3xl p-6 space-y-4">
            <div className="flex items-center justify-between text-xs font-semibold tracking-wider text-apple-silver uppercase">
              <span>OR enter Charger ID</span>
              <span className="text-[10px] text-white/30 lowercase">(e.g. CM-CMOD0135-VLEM)</span>
            </div>
            ...
            <button 
              ...
              className="w-full bg-white text-black font-semibold py-4 rounded-2xl shadow-lg transition active:scale-95 disabled:opacity-50 flex items-center justify-center space-x-2 text-sm"
            >
              {loading ? (
      ...
      {screen === "connector" && stationDetails && (
        <div className="w-full space-y-6 animate-fadeIn">
          ...
          <div className="glass rounded-[32px] p-6 space-y-4 relative overflow-hidden">
            ...
            <div className="flex items-start space-x-4">
              ...
              <div className="space-y-1">
                ...
                <h2 className="font-bold text-xl text-white tracking-tight">{stationDetails.charger_name}</h2>
                <p className="text-xs text-apple-silver font-light leading-relaxed">{stationDetails.location_name}</p>
              </div>
            </div>
            <div className="flex items-center space-x-2 border-t border-white/5 pt-4 text-xs text-apple-silver">
          ...
          <div className="space-y-3">
            <h3 className="text-xs font-bold uppercase tracking-wider text-apple-silver px-1">Select Connector (Gun)</h3>
                  ...
                  <div 
                    ...
                    className={`w-full p-5 rounded-3xl cursor-pointer transition flex items-center justify-between border ${
                      isSelected 
                        ...
                        : "border-white/15 glass hover:bg-white/5"
                    } ${!isAvailable && "opacity-40 cursor-not-allowed"}`}
                  >
                    <div className="flex items-center space-x-4">
                      <div className={`h-10 w-10 rounded-full flex items-center justify-center text-sm font-bold ${
                        isSelected ? "bg-apple-accent text-white" : "bg-white/10 text-white/70"
                      }`}>
                        {con.connector_id}
                      </div>
                      <div>
                        <div className="font-semibold text-sm tracking-wide text-white">{con.gun_label}</div>
                        <div className="text-[11px] text-apple-silver mt-0.5">{con.type} • {con.max_power_kw} kW</div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-[11px] font-semibold text-white/90">{con.status}</span>
                    </div>
                  </div>
                );
              })}
      ...
      {screen === "charging" && activeSession && (
        <div className="w-full space-y-8 animate-fadeIn">
          ...
          <div className="w-full flex flex-col items-center justify-center mt-6">
            <div className="relative h-64 w-64 rounded-full flex items-center justify-center border border-white/5 bg-gradient-to-b from-white/[0.02] to-transparent shadow-2xl p-6">
              ...
              <div className="text-center space-y-1 z-10">
                ...
                <div className="flex items-baseline justify-center">
                  ...
                  <span className="text-sm font-semibold text-apple-silver ml-1">kWh</span>
                </div>

                <div className="h-[1px] w-12 bg-white/10 mx-auto my-2" />

                {/* Running Cost */}
                <div className="flex items-center justify-center space-x-1 text-sm font-semibold text-white/90">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="glass rounded-3xl p-5 space-y-1 flex flex-col justify-center">
              <div className="flex items-center space-x-1.5 text-xs text-apple-silver font-medium">
          ...
          <div className="glass rounded-3xl p-6 space-y-5">
            <div className="flex items-center space-x-2 text-xs text-apple-silver leading-relaxed font-light">
      ...
      {screen === "receipt" && receipt && (
        <div className="w-full space-y-6 animate-fadeIn flex flex-col">
          ...
          <div className="flex items-center w-full justify-between mb-2">
            <button 
              onClick={handleReset}
              className="h-10 w-10 bg-white/5 border border-white/10 rounded-full flex items-center justify-center text-apple-silver hover:text-white transition active:scale-95 shadow-md"
            >
          ...
          {/* Location Name Card */}
          <div className="w-full glass rounded-[24px] p-5 border border-white/10 relative overflow-hidden">
            <div className="absolute top-0 right-0 h-16 w-16 bg-apple-accent/5 rounded-full blur-xl pointer-events-none" />
            <div className="space-y-3">
              <div>
                <span className="text-[10px] font-bold text-white/40 uppercase tracking-widest block">Location Name</span>
                <span className="font-extrabold text-base text-white tracking-tight mt-0.5 block leading-tight">
                  {receipt.location_name || "OCPI Test Location - PROD"}
                </span>
              </div>
              <div className="pt-2 border-t border-white/5">
                <span className="text-[10px] font-bold text-white/40 uppercase tracking-widest block">Charger Name</span>
                <span className="font-bold text-sm text-white tracking-tight mt-0.5 block">
          ...
          <div className="grid grid-cols-3 gap-3">
            {/* Energy Usage Fee */}
            <div className="glass rounded-[20px] p-3 border border-white/15 text-center flex flex-col justify-between h-24 relative overflow-hidden">
          ...
          {/* Total Amount Card */}
          <div className="w-full glass rounded-[20px] p-5 border border-white/10 text-center relative overflow-hidden">
            <div className="absolute top-0 right-0 h-24 w-24 bg-apple-accent/5 rounded-full blur-2xl pointer-events-none" />
          ...
          <div className="w-full glass rounded-[20px] p-4 border border-white/10 text-center relative overflow-hidden">
            <span className="text-[10px] font-extrabold text-white/50 uppercase tracking-widest block">
              Total Energy Usage
            </span>
            <span className="text-2xl font-extrabold text-white/90 tracking-tight mt-1 block">
              {(receipt.energy_kwh || 0.00).toFixed(2)} kWh
            </span>
          </div>

          {/* Session Telemetry Details Table */}
          <div className="w-full glass rounded-[24px] p-5 border border-white/10 space-y-3">
            <div className="flex justify-between items-center text-xs">
              <span className="font-semibold text-white/40 uppercase tracking-wider">Vehicle Model</span>
              <span className="font-bold text-white tracking-wide">{receipt.vehicle_model || "--"}</span>
            </div>
            <div className="flex justify-between items-center text-xs pt-2.5 border-t border-white/5">
              <span className="font-semibold text-white/40 uppercase tracking-wider">Stop Reason</span>
              <span className="font-bold text-white tracking-wide">{receipt.stop_reason || "Stopped Remotely"}</span>
            </div>
            <div className="flex justify-between items-center text-xs pt-2.5 border-t border-white/5">
              <span className="font-semibold text-white/40 uppercase tracking-wider">Start Time</span>
              <span className="font-bold text-white tracking-wide">{receipt.start_time_formatted || "09:02 PM"}</span>
            </div>
            <div className="flex justify-between items-center text-xs pt-2.5 border-t border-white/5">
              <span className="font-semibold text-white/40 uppercase tracking-wider">End Time</span>
              <span className="font-bold text-white tracking-wide">{receipt.end_time_formatted || "09:07 PM"}</span>
            </div>
          </div>
          ...
          <div className="w-full text-center space-y-1.5 mt-2 mb-1">
            ...
            <div className="flex items-center justify-center space-x-2">
              ...
              <button 
                ...
                className="h-7 w-7 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 flex items-center justify-center text-apple-silver hover:text-white transition active:scale-90"
          ...
          <div className="w-full glass bg-apple-emerald/5 border border-apple-emerald/20 p-4 rounded-[20px] flex items-start space-x-3 text-xs leading-relaxed">
            ...
            <div className="space-y-1 text-white/70">
              <span className="font-bold text-white block">Refund Initiated instantly!</span>
              <p className="font-light">
                Authorized: <span className="font-bold text-white">₹{(receipt.prepaid_amount || 0.00).toFixed(2)}</span> • Charged: <span className="font-bold text-white">₹{(receipt.actual_cost || 0.00).toFixed(2)}</span>
              </p>
          ...
          <div className="w-full space-y-3 pt-2">
            ...
            <button 
              onClick={handleReset}
              className="w-full bg-white text-black font-bold py-4 rounded-2xl transition active:scale-95 flex items-center justify-center space-x-2 text-sm shadow-md"
            >
      ...
      {screen === "support" && (
        <div className="w-full space-y-6 animate-fadeIn">
          <div className="glass rounded-[32px] p-6 space-y-4">
            <h2 className="font-extrabold text-2xl tracking-tight text-white">UPICharge Support</h2>
            <p className="text-xs text-apple-silver font-light">
              Facing issues with a charger? Our operations desk can help verify, trigger, or force-stop sessions manually.
            </p>
          </div>

          <div className="glass rounded-3xl p-6 space-y-4">
            <h3 className="text-xs font-bold uppercase tracking-wider text-apple-silver">Direct Live Helplines</h3>
            <div className="space-y-2 text-sm">
              <a 
                href="https://wa.me/919999999999" 
                target="_blank" 
                rel="noreferrer"
                className="w-full p-4 rounded-2xl border border-white/10 glass hover:bg-white/5 flex items-center justify-between transition"
              >
                <div className="flex items-center space-x-3">
                  ...
                  <span className="font-semibold text-white">WhatsApp Business Support</span>
                </div>
                <ArrowRight className="h-4 w-4 text-apple-silver" />
              </a>

              <div className="w-full p-4 rounded-2xl border border-white/10 glass flex flex-col justify-start">
                <span className="text-[10px] text-apple-silver uppercase tracking-wider font-bold">Helpline Desk</span>
                <span className="text-white font-bold text-lg mt-1">+91 9999 999 999</span>
                <p className="text-xs text-apple-silver mt-1 font-light">Available 24x7 for physical charging site assistance.</p>
              </div>
            </div>
          </div>
          ...
          <button 
            ...
            className="w-full bg-white text-black font-bold py-4 rounded-2xl transition active:scale-95 text-sm"
          >
            Back to Scanner
          </button>
        </div>
      ...
      {screen === "map" && (
        <div className="w-full space-y-6 animate-fadeIn">
          <div className="glass rounded-[32px] p-6 space-y-4">
            <h2 className="font-extrabold text-2xl tracking-tight text-white">Charger Locator Map</h2>
            <p className="text-xs text-apple-silver font-light">
          ...
          <div className="glass rounded-3xl p-5 space-y-4">
            <div className="h-64 bg-white/5 border border-white/10 rounded-2xl relative overflow-hidden flex items-center justify-center text-center p-6">
              ...
              <div className="space-y-2 z-10 flex flex-col items-center">
                ...
                <h4 className="font-bold text-white">Interactive Map Widget</h4>
                <p className="text-xs text-apple-silver px-4 font-light">
                  Leaflet rendering in Dark Mode. Near CMOD0135 (Kerala, India).
                </p>
              </div>
            </div>

            <div className="space-y-2">
              <div className="glass-light p-4 rounded-2xl flex items-start justify-between">
                <div className="space-y-0.5">
                  <span className="text-xs font-bold text-white">Station CMOD0135</span>
                  <p className="text-[11px] text-apple-silver">1.2 km away • CCS2, Type 2, 15A Available</p>
                </div>
                <button 
                  onClick={() => {
                    setQrInput("CMOD0135");
      ...
      {screen !== "charging" && (
        <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[85%] max-w-[340px] h-16 glass rounded-full flex items-center justify-around px-4 z-40 shadow-xl shadow-black/80">
          <button 
            onClick={() => { stopScanner(); setScreen("home"); }}
            className={`flex flex-col items-center justify-center space-y-1 ${screen === "home" ? "text-apple-accent" : "text-apple-silver hover:text-white"}`}
          >
          ...
          <button 
            ...
            className={`flex flex-col items-center justify-center space-y-1 ${screen === "map" ? "text-apple-accent" : "text-apple-silver hover:text-white"}`}
          >
          ...
          <button 
            ...
            className={`flex flex-col items-center justify-center space-y-1 ${screen === "support" ? "text-apple-accent" : "text-apple-silver hover:text-white"}`}
          >
